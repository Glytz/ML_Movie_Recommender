...
def calculate_probabilities(dataframe, values_name="rating", column_name=None, column_key=None):
    df_votes = dataframe
    if column_name != None:
    ...
    matrix_votes = matrix_votes.reindex(range(1, dataframe.groupby(["item.id"]).size().shape[0] + 1))
...
def similarities(dataframe, job, gender, age):
    df = dataframe
    p_v = calculate_probabilities(df, "rating")

    p_v_j = calculate_probabilities(df, "rating", "job", job)
    p_j = calculate_probabilities(df, "job")
    job_idx = get_job_idx(job, df)
    p_j = p_j[:, job_idx]

    p_v_g = calculate_probabilities(df, "rating", "gender", gender)
    p_g = calculate_probabilities(df, "gender")
    gender_idx = get_gender_idx(gender, df)
    p_g = p_g[:, gender_idx]

    age_group = age // 10 + 1
    p_v_a = calculate_probabilities(df, "rating", "age_group", age_group)
    p_a = calculate_probabilities(df, "age_group")
    age_group_idx = get_age_group_idx(age_group, df)
...
def test():  
    ...
    for i in range(len(test)):
        #compute the user probability to like the film
        sub_test = test[i]
       # result = similarities(train, )

    ratings = similarities(df, "student", "M", 27)
    printHighestValue(ratings, uItem, 10)
    ratings = similarities(df, "student", "F", 27)
    printHighestValue(ratings, uItem, 10)
...
for i in range(1, number_of_users):
    ...
    rating = similarities(train, occupation, sex, age)
    y = 0
    print(i)
